...
export default async function SiteProfilePage({ params }: Props) {
  ...
  // Query by `domain` — the live schema has no `slug` column
  const { data: site } = await supabase
    .from('websites')
    .select('*')
    .eq('domain', decodeURIComponent(slug))
    .single();

  if (!site) notFound();

  // win tallies — column is `category` (text), not `category_id`
  const { data: tallies } = await supabase
    .from('website_win_tallies')
    .select('category, win_count, last_win_week_start, streak_weeks')
  ...
  // reviews — live schema has `sentiment` + `body`, not `pros`/`cons` arrays
  ...
  const image = site.favicon_url || site.og_image;
  const displayName = site.title || site.domain;
    ...
    <div className="min-h-screen bg-gray-950 text-white">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-start gap-6 mt-4">
            <div className="w-20 h-20 rounded-2xl bg-gray-800 border border-gray-700 flex items-center justify-center text-3xl shadow-xl">
              {image
                ? <img src={image} alt="" className="w-12 h-12 rounded-xl" />
                : '🌐'}
            </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-10 space-y-12">
        {tallies && tallies.length > 0 && (
          <section>
            <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">🏆 Win Tallies by Category</h2>
              ...
              {tallies.map(t => (
                <div key={t.category}
                  ...
                  {t.last_win_week_start && (
                    <p className="text-xs text-gray-600 mt-1">Last win: {new Date(t.last_win_week_start).toLocaleDateString()}</p>
                  )}
                </div>
              ))}
            </div>
        ...
        {achievements && achievements.length > 0 && (
          <section>
            <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">🎖️ Achievements</h2>
            <div className="flex flex-wrap gap-3">
              {achievements.map((a, i) => {
                const meta = ACHIEVEMENT_META[a.achievement_type] ?? { icon: '🏅', label: a.achievement_type, color: 'text-gray-300' };
                return (
                  <div key={i}
        ...
        <section>
          <h2 className="text-xl font-semibold mb-6 flex items-center gap-2">💬 Community Reviews</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              <h3 className="text-green-400 font-semibold mb-3">✅ Pros ({pros.length})</h3>
              {pros.length === 0
                ? <p className="text-gray-600 text-sm">No pros submitted yet.</p>
                : <ul className="space-y-3">{pros.map(r => (
                    <li key={r.id} className="bg-gray-900 border border-green-900/40 rounded-xl p-4 text-sm text-gray-300">{r.body}</li>
                  ))}</ul>}
            </div>
            <div>
              <h3 className="text-red-400 font-semibold mb-3">❌ Cons ({cons.length})</h3>
              {cons.length === 0
                ? <p className="text-gray-600 text-sm">No cons submitted yet.</p>
                : <ul className="space-y-3">{cons.map(r => (
                    <li key={r.id} className="bg-gray-900 border border-red-900/40 rounded-xl p-4 text-sm text-gray-300">{r.body}</li>
                  ))}</ul>}
            </div>
          </div>
        </section>
